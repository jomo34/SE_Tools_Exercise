...
public class LoginServer
{
   ...
   public static void activity(String time1[][],int N ) {
	   ...
	   Arrays.sort(time1, new Comparator<String[]>() {      
		...
		for(int i = 0; i < N; i++) {
			...
			// 직전 종료시간이 다음 회의 시작 시간보다 작거나 같다면 실행
   ...
   {
      String DB_URL = "jdbc:mysql://localhost:3306/algorithm?serverTimezone=UTC"; //접속할 DB 저버
       Connection conn=null;//mysql 접속용
       Statement state=null;//mysql 연결
       String uname=null;
       try {
          conn=DriverManager.getConnection(DB_URL,"root","990302"); //db주소와, 사용자, 비밀번호를 통해서 접근
   ...
   private static class Handler implements Runnable 
   {
      ...
      private String logid=""; //로그아이디 저장 변수
       String DB_URL = "jdbc:mysql://localhost:3306/algorithm"; //접속할 DB서버
         Connection conn=null;//mysql 접속용
         Statement state=null;//mysql 연결
      ...
      public void run() 
      {
         ...
         conn=DriverManager.getConnection(DB_URL,"root","990302"); //db주소와, 사용자, 비밀번호를 통해서 접근
         state=conn.createStatement(); //mysql 연결
         boolean realuser=false; //실제 사용자가 있는지 확인하기 위한 변수
         ...
         System.out.println(input);
            if(input.startsWith("logid")) //client에서 logid가 왔을때 로그인
            {
               ...
               ResultSet rs=state2.executeQuery(sql); //결과값 저장
               ...
               while(rs.next()) //데이터베이스 스캔
               {
                  ...
                  if(pw.equalsIgnoreCase(dbpw)) //입력한 비밀번호와 아이디가 맞을 때, 로그인 성공
                  {
            ...
            else if(input.startsWith("Resister")) //회원가입
            {
               ...
               userlist.add(nw[3]); //가입하면, hashset에 userid추가
               out.println("Welcome"); //회원가입되었다는것을 보내줌
               state4.close();
            }
            else if(input.startsWith("Update")) {  // 내정보 변경
               String[] ch_info = input.split(":");
               try {
	               String ch_sql = "update user set email = " + "'"+ch_info[2]+"',"+"grade = "+ch_info[3]+" where userid = " +"'"+ch_info[1]+"'";// 닉네임과 상태메시지 변경
            ...
            else if(input.startsWith("Book"))// 예약받아서 저장
            {
    			...
    			time1[N][1]=String.valueOf(d);//종료ㅕ시간
			...
			if(Integer.parseInt(scheduler[0])==15 && schedulecount==0 && Integer.parseInt(scheduler[1])==37) // 6시 되면 한번만 돌아감 
			{
				activity(time1,N);
